...
| D-05 | **`--output` flag to print injected content to stdout** | Useful for piping into other tools, reviewing in CI, or pasting manually. Composability is a hallmark of good CLI design. | XS | `npx @hudyv2298/ui-hierarchy-mcp --init --output` prints the block to stdout without writing any files. |
...
\`\`\`bash
claude mcp add ui-hierarchy -- npx -y @hudyv2298/ui-hierarchy-mcp
\`\`\`
...
{
  "mcpServers": {
    "ui-hierarchy": { "command": "npx", "args": ["-y", "@hudyv2298/ui-hierarchy-mcp"] }
  }
}
\`\`\`
